...
def main():
    ...
    ax.set_title(f"Boxplot de {selected_var}",fonsize=12)
    st.pyplot(fig)
    ...
    if df[selected_var].nunique() < 10:
        ...
        ax.set_title(f"Distribución de {selected_var}",fonsize=12)
        st.pyplot(fig)
    ...
    if df[selected_var].dtype in ["int64", "float64"] and selected_var != "quality":
        ...
        ax.set_title(f"Relación entre {selected_var} y Calidad",fonsize=12)
        st.pyplot(fig)
